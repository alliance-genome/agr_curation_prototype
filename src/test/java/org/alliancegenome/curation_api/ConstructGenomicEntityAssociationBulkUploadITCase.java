...
@DisplayName("202 - Construct Genomic Entity Associations bulk upload")
@Order(202)
public class ConstructGenomicEntityAssociationBulkUploadITCase extends BaseITCase {
	...
	private void loadRequiredEntities() throws Exception {
		...
		construct = getConstruct(constructModEntityId);
	}
	...
	public void constructGenomicEntityAssociationBulkUploadCheckFields() throws Exception {
		...
		RestAssured.given().
			when().
			get(constructGenomicEntityAssociationGetEndpoint + "?constructId=" + construct.getId() + "&relationName=" + relationName + "&genomicEntityId=" + gene.getId()).
			then().
			statusCode(200).
			body("entity.relation.name", is(relationName)).
			body("entity.constructGenomicEntityAssociationObject.primaryExternalId", is(geneModEntityId)).
			body("entity.constructAssociationSubject.primaryExternalId", is(constructModEntityId)).
		...
		RestAssured.given().
			...
			then().
			statusCode(200).
			body("entity.primaryExternalId", is(constructModEntityId)).
			...
			body("entity.constructGenomicEntityAssociations[0].constructGenomicEntityAssociationObject.primaryExternalId", is(geneModEntityId)).
